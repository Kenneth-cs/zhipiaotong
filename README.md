...
- **使用接口**：火山引擎 `OCRPdf` (智能文档解析) + `DeepSeek-V3` 大语言模型
- **识别流程**：
  1. 发票图片/PDF → Base64编码
  2. 调用火山引擎 OCRPdf 接口提取包含版面还原的 Markdown 文本
  3. 执行发票类型前置判定 (`detectInvoiceType`) 区分全电、成品油、专票、普票等
  4. 利用大模型（DeepSeek V3）并结合发票分类 Prompt 精准提取关键字段（支持标签锚点解析，避免行序干扰）
  5. 后端执行**财务逻辑自愈**（校验 `金额 + 税额 = 价税合计`），自动解决 OCR 数字 "1" 和 "7" 的混淆问题，并参考大写金额进行兜底纠错
...
- **措施1（核心等式校验）**：利用 `金额 + 税额 = 价税合计` 进行校验。若等式不成立，程序尝试自动替换嫌疑数值中的“1”和“7”，若替换后等式完美成立（如 `47.43 + 0.47 = 47.90`），则自动纠偏。
- **措施2（税率验证）**：验证纠正后的数字是否符合国内常见发票税率（1%, 3%, 6%, 9%, 13% 等）。
- **措施3（大写金额兜底）**：将高准确率的中文大写金额转换为数字，作为“价税合计”的绝对标尺反推小写数字。

### 实施优先级

| 优先级 | 内容 | 文件 | 状态 |
|--------|------|------|------|
| P0 | 标签锚点修复购销方对调 | `server/volcengine.ts` | ✅ 已完成 |
| P0 | 数字“1”和“7”财务逻辑纠错 | `server/volcengine.ts` | ✅ 已完成 |
| P1 | 发票类型分类前置逻辑 | `server/volcengine.ts` | ✅ 已完成 |
| P2 | 非全电发票专属字段映射 | `server/volcengine.ts` | ✅ 已完成 |
| P3 | 前端可疑字段标记 | 前端列表组件 | 待优化 |

---

## 项目改进与反思 (v1.1)

本次 v1.1 升级中我们完成了品牌更名（小财犬）、发票分类增强解析、金额混淆财务自愈、以及兑换码与文件夹管理功能的完整开发。

**潜在问题与后续优化建议**：
1. **大模型延迟与成本考量**：目前发票信息提取强依赖大语言模型（DeepSeek），对于大量并发请求可能存在一定延迟并消耗一定 token 成本。后续可以沉淀高质量的数据集，训练一个针对发票结构化解析的小参数量专用模型，或进一步强化正则引擎拦截标准版式的发票，仅对复杂版式启用 LLM。
2. **大文件并发限制**：当用户同时批量上传大量高分辨率 PDF 发票时，尽管当前设置了队列控流（`MAX_CONCURRENT`），依然存在占用大量服务器内存的问题。建议引入流式解析或者在前端进行尺寸压缩与切片。
3. **文件夹共享及权限**：当前发票归类于个人文件夹，未来若要满足“数字员工（帮我管理财税）”和中大型企业协同的愿景，应考虑引入**企业空间/企业工作台**概念，增加发票数据的 RBAC 权限管理，让团队之间能够共享发票管理视图。
